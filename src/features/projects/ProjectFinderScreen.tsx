...
const SectionHeader = ({ title }: { title: string }) => <Text style={styles.sectionTitle}>{title}</Text>;
...
const ButtonSelector = ({ options, selectedValue, onSelect, multiSelect = false }: {
    ...
    return (
        <View style={styles.buttonSelectorContainer}>
            {options.map(option => (
                <TouchableOpacity key={option} style={[styles.selectorButton, isSelected(option) && styles.selectorButtonActive]} onPress={() => handleSelect(option)}>
...
const ProjectFinderScreen = () => {
    ...
    const handleSearch = () => {
        // This now navigates to the beautiful comparison deck, which will use its own temporary data.
        navigation.navigate('ProjectComparisonDeck', { requirements });
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollView}>
                ...
                <View style={styles.formSection}>
                    <SectionHeader title="Location" />
                    <TextInput label="City or Area (e.g., Mira Road)" mode="outlined" style={styles.input} />
                </View>
                <View style={styles.formSection}>
                    <SectionHeader title="Configuration" />
                    <ButtonSelector
                        options={['1 BHK', '2 BHK', '3 BHK', '4+ BHK']}
                        selectedValue={requirements.configurations}
                        onSelect={(val) => handleInputChange('configurations', val)}
                        multiSelect
                    />
                </View>
                <Button mode="contained" onPress={handleSearch} style={styles.submitButton}>
                    Show Matching Projects
                </Button>
            </ScrollView>
        </SafeAreaView>
...
const styles = StyleSheet.create({
    ...
    appbarTitle: { fontWeight: 'bold', color: '#111827' },
    scrollView: { padding: 20 },
    headerText: { fontSize: 16, color: '#6B7280', marginBottom: 24 },
    formSection: { marginBottom: 24 },
    sectionTitle: { fontSize: 18, fontWeight: '600', color: '#1F2937', marginBottom: 12 },
    input: { backgroundColor: '#F9FAFB' },
    submitButton: { backgroundColor: '#4F46E5', borderRadius: 12, paddingVertical: 8 },
    buttonSelectorContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
